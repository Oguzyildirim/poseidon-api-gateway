...
export const ACTION_TYPES = {
  SEARCH_SHIPS: 'ship/SEARCH_SHIPS',
...
export default (state: ShipState = initialState, action): ShipState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_SHIPS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_SHIPS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_SHIPS):
...
const apiSearchUrl = 'api/_search/ships';

// Actions

export const getSearchEntities: ICrudSearchAction<IShip> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_SHIPS,
  payload: axios.get<IShip>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
